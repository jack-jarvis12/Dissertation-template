...
\begin{itemize}
    ...
    \item Natural methods of everyday micro interactions (e.g. clicking through a slideshow or skipping a song) without tethering to traditional input devices such as keyboards, mice, or controllers.
...
\autoref{tab:acc_summary} contains a summary of the performance differences of the two LED viewing angles across the different experiments. See \autoref{chapter:experiment} for more detailed results.

\begin{table}[htp]
    \centering
    \begin{tabular}{|l|l|l|}
        \hline
        \textbf{Experiment} & $\pm 12^\circ$ LED Accuracy & $\pm 60^\circ$ LED Accuracy \\
        \hline
        \textbf{All Features, All Gestures} & 81.6\% & 78.2\% \\
        \hline
        \textbf{All Features, Excluding Pinch Gestures} & 87.0\% & 84.1\% \\
        \hline
        \textbf{Smartwatch Features (Dorsal Side Only)} & 67.3\% & 61.1\% \\
        \hline
        \textbf{Transmission Features (Band Width 3)} & 58.0\% & 31.4\% \\
        \hline
        \textbf{Transmission Features (Band Width 5)} & 20.8\% & 10.4\% \\
        \hline
    \end{tabular}
    \caption{Summary of the classification accuracies for both LED viewing angles. ("Transmission Features" refers to data recorded from photodiodes physically far from the active LED. "Band Width" refers to the number of sensors excluded from the readings. See \autoref{section:reflection_transmission} for more detail.)}
...
\subsection{LEDs}
A diode is an electrical component that only allows the flow of current in one direction. An LED is a diode that converts electrical energy into light. The angle of half intensity, or viewing angle, of an LED is the angle at which the intensity of the light is half the maximum intensity of the LED. 

Radiant power is the total amount of optical power emitted from an LED. Radiant intensity is the radiant power per solid angle. LEDs with the same radiant power but smaller viewing angles have higher radiant intensities.

\subsection{Photodiodes}
Photodiodes are semiconductors that convert light into electrical energy. When light hits a photodiode, it generates a small current.
...
where \(I_{in}\) is the input current and \(R_f\) is the value of the feedback resistor.

\section{Microcontrollers}
Microcontrollers are small single board computers. Code can be run on microcontrollers to control other electrical components through output pins, and to read signals through input pins. Most microcontrollers have analogue-to-digital converters (ADCs), which convert analogue signals at the input pins to binary numbers that the processor can manipulate. The conversion rate of an ADC is how fast it can convert an analogue value to a digital value. The resolution of an ADC is the smallest change in an analogue signal that would result in a change in the digital output.
...
\section{Biology}
Three main phenomena occur when IR light interacts with biological tissue: \textbf{reflection, absorption }and\textbf{ scattering.}
...
The wristband connects to a Teensy 4.1 microcontroller via an FFC cable, which is plugged into a connector on the head. The microcontroller activates the LEDs and reads from the photodiodes as described in \autoref{sec:operating_principle}.
...
\autoref{listing:capture_loop} shows code which runs on the Teensy. The code sequentially activates the LEDs. For each active LED, it records an average of five readings for each photodiode.
...
\subsection{Op-amps}
Initial designs of the wristband used one OPA381 op-amp for each segment. OPA381s are high precision, low noise op-amps that work well in transimpedance amplifiers with photodiodes. However, the OPA381s were significant contributors to the overall cost of the wristband. To reduce the cost, the final design uses the TLV9062. The TLV9062 is a cheaper, general purpose amplifier with higher input bias current and offset voltage. The TLV9062 is also a dual amplifier, with each chip containing two op-amps. In the final wristband design, two adjacent segments share a single TLV9062 chip, shown in \autoref{fig:segments_new}. This halves the number of components needed, significantly reducing the cost.
...
We collected data from 1 participants. Each participant completed two sessions, one for each type of LED. A session involves performing 20 trials of 20 gestures, with the order of gestures in each batch randomised. The order in which users completed the sessions was counterbalanced to avoid order effects such as fatigue.
...
The software used to collect data was adapted from code provided by GestIR\cite{diamand2025}, and is shown in \autoref{fig:software}. OpenCV was used to display the gesture for users to imitate. The live capture matrix was also displayed, which allowed for easy debugging of wristband segments.
...
\section{Findings}
This section details the performance of the the models trained to classify the hand gestures. For all statistical tests, a significance level of $\alpha=0.05$ was  used.
...
\autoref{fig:accuracy_all} shows the average accuracy of MLPs with Monte Carlo dropout trained on data from the $\pm 12^\circ$ and $\pm 60^\circ$ wristbands across all participants. To account for variation introduced by data partitioning, we trained five models on different random splits of the data. The bars show the mean model performance, with error bars representing the standard deviation. 
...
A paired $t$-test was conducted to compare the average model performance between the two wristbands. Overall, there was no statistical significance between the two wristbands ($t$ = 1.758, $p$ = 0.109, Cohen's $d$ = -0.530).
...
For the $\pm 60^\circ$ wristband, a paired $t$-test between the accuracies of the models before and after removing the segments revealed the performance drop was significant ($t$ = 2.966, $p$ = 0.031, Cohen's $d$ = -1.211). The average performance decrease was 1.69\%.

For the $\pm 12^\circ$ wristband, the accuracy drop was not statistically significant ($t$ = 2.062, $p$ = 0.094, Cohen's $d$ = 0.842). However, the huge increase in standard deviation for the models trained on participant 3 (shown by the error bars in \autoref{fig:broken_segments_graph}) shows that removing the segments produced less robust models.
...
To check for order effects, a Welch's unpaired $t$-test was performed on the performance difference between the wristbands for each participant. This was not statistically significant ($t$ = 2.136, $p$ =0.063, Cohen's $d$ =1.300), showing that there is no evidence that the order participants completed the trials affected the results.
...
We conducted a Kruskal-Wallis H test on the accuracies of models trained on participants for the three locations. Due to the small sample size of participants in locations B and C, an additional permutation test was performed. This was repeated for both wristbands.
...
The confusion matrices in \autoref{fig:cf_all} show the predicted and true labels for gestures in the test set. Like SensIR \cite{McIntosh2017SensIR:Reflection} and GestIR \cite{diamand2025}, the majority of misclassifications are from the four pinch gestures (pinch\_index, pinch\_middle, pinch\_ring, pinch\_pinkie). This suggests that gestures involving precise finger movement with similar overall hand positions are difficult for IR systems to detect.

Excluding the pinch gestures, the $\pm 12^\circ$ and $\pm 60^\circ$ wristbands achieve average accuracies of 87.0\% and 84.1\% respectively.
...
The total datasets for each participant contain 20 trials of 20 gestures, resulting in 400 captures. The test, train validation split of 70\%, 10\% and 10\% produces a training set size of 280 captures. To evaluate the impact of reducing the dataset size, we trained models using stratified subsamples of the test set.

Models were trained on training sets of size 70, 140, 210, and 280 captures, with fixed validation and test sets. This was repeated for five test/train/validation splits.
...
\label{section:reflection_transmission}
...
We trained models using data with only reflection features with a band width of 3. A paired $t$-test between these accuracies and the accuracies of models trained on all data wasn't significant for either wristband ($t$ = -0.461, $p$ = 0.655, Cohen's $d$ = 0.139 for the $\pm 12^\circ$ wristband and $t$ = -0.244, $p$ = 0.812, Cohen's $d$ = 0.074 for the $\pm 60^\circ$ 
wristband). This indicates reflection features generated from photodiodes physically close to the active LEDs provide sufficient information and that transmission features may not be necessary for gesture recognition.

\subsubsection{Transmission Features}

When training on transmission features only, the overall accuracy drops significantly. However, the accuracy of the $\pm 12^\circ$ LED wristband is much higher than the $\pm 60^\circ$ wristband.
...
\autoref{fig:accuracy_transmission_3} shows  the average accuracy of MLPs with Monte Carlo dropout trained on data for both wristbands, using only transmission features with a diagonal band of width 3 removed from the captures. The $\pm 12^\circ$ wristband achieves an average accuracy of 58.0\%, and the $\pm 60^\circ$ wristband achieves 31.4\%.

We conducted a paired $t$-test to compare the average accuracies of each wristband. This showed a significant difference ($t$ = 5.362, $p$ < 0.001, Cohen's $d$ = -1.617).
...
\autoref{fig:accuracy_transmission_5} shows the average accuracy of models trained on transmission features with a band width of 5. The average accuracy of the $\pm 12^\circ$ is 20.8\%, and the average accuracy of the $\pm 60^\circ$ wristband is 10.4\%. A paired $t$-test reveals the accuracy of the $\pm 12^\circ$ wristband is significantly higher ($t$ = 3.709, $p$ = 0.004, Cohen's $d$ = -1.118). Again, the accuracy of both wristbands is lower than when training with all features.
...
\autoref{fig:accuracy_smartwatch} shows the average accuracy of models trained using only data from photodiodes 8, 9, 10, and 11. The $\pm 12^\circ$ wristband had an average accuracy of 67.3\% and the $\pm 60^\circ$ wristband had an average accuracy of 61.1\%. This is a performance drop of 14.3\% and 17.1\% compared to models trained on all segments. A paired $t$-test between the accuracies for both wristbands revealed that the $\pm 12^\circ$ wristband performs significantly better on smartwatch features ($t$ = 2.773, $p$ = 0.020, Cohen's $d$ = -0.836). 
...
Wrist circumference was measured 1.5 cm from the centre of the ulnar styloid (the same place the wristbands were worn during data collection) with the fist clenched. \autoref{fig:accuracy_vs_circumference} shows scatter plots comparing model accuracies with wrist circumferences for models trained on all features. Pearson correlation tests between the accuracies and wrist circumferences revealed no significant correlation for either wristband ($r$ = -0.112, $p$ =0.743 for the $\pm 12^\circ$ wristband and $r$ =0.056, $p$ =0.869 for $\pm 60^\circ$ wristband). Therefore, there is no evidence of a linear relationship between wrist circumference and accuracy. There was also no significant correlation for transmission features or smartwatch features. Scatter plots for models trained on these features can be found in \autoref{appx:acc_vs_wrist_size}.
...
\begin{table}[htp]
    ...
    \begin{tabular}{|l|l|l|}
        ...
        \textbf{Experiment} & $\pm 12^\circ$ LED Accuracy & $\pm 60^\circ$ LED Accuracy \\
        \hline
        \textbf{All Features, All Gestures} & 85.0\% & 83.3\% \\
        \hline
        \textbf{All Features, Excluding Pinch Gestures} & 90.4\% & 89.19\% \\
        \hline
    \end{tabular}
...
\begin{itemize}
    ...
    \item Transimpedance amplifier parameters 
\end{itemize}
