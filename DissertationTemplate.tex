...
$p_{adj}$ &: & $p$-value adjusted for multiple comparisons \\
$t$ &: & Test statistic for a $t$-test \\
$d$ &: & Cohen's $d$ - effect size \\
$H$ &: & Test statistic for a Kruskal Wallis test \\
...
\label{chap:context}
Gesture recognition systems offer the potential to harness hand gestures for control over computer systems, allowing users to interact with technology through intuitive, natural body movements rather than traditional input devices. There are many methods of hand gesture recognition, including computer vision systems, acoustic sensing, and electromyography (EMG), but many devices lack the accuracy, ergonomics, and practicality needed for everyday use.

Infrared (IR) based sensing stands out as an accurate, practical method of gesture recognition that is less susceptible to sensor placement and skin coupling issues than other methods \cite{diamand2025}. IR sensing systems are also relatively cheap and low power compared to other methods of gesture detection, and the possibility of future integration into smartwatch style devices offers a practical, unobtrusive method of integrating gestures into everyday human-computer interaction \cite{McIntosh2017SensIR:Reflection}.
...
This project makes multiple key contributions:
\begin{itemize}
...
\begin{table}[htp]
    ...
    \begin{tabular}{lrr}
        ...
        Smartwatch Features (Dorsal Side Only), 20 Gestures & 67.3\% & 61.1\% \\
        Transmission Features (Band Width 3), 20 Gestures & 58.0\% & 31.4\% \\
        Transmission Features (Band Width 5), 20 Gestures & 20.8\% & 10.4\% \\
        \bottomrule
    \end{tabular}
...
\begin{figure}[h]
     ...
     \begin{subfigure}[b]{0.4\textwidth}
         \centering
         \includegraphics[width=\textwidth]{GestIR.png}
         \caption{GestIR \cite{diamand2025}}
        \label{fig:gestIR_wristband}
     \end{subfigure}
      \begin{subfigure}[b]{0.3\textwidth}
         \centering
         \includegraphics[width=\textwidth]{SensIR_wristband.png}
        \caption{SensIR \cite{McIntosh2017SensIR:Reflection}}
        \label{fig:sensIR_wristband}
     \end{subfigure}
    \caption{Previous infrared sensing wristbands}
    \label{fig:sensIR_and_gestIR}
\end{figure}

GestIR and SensIR both used bulky, hand soldered through hole components, shown in \autoref{fig:sensIR_and_gestIR}. Repeated flexing caused broken solder joints, and physical space constraints meant additional PCBs were required to house electrical components. Also, suitable through hole LEDs were difficult to source. This meant there were limited LEDs available to test, and complex manufacturing processes such as dedoming of LEDs were required.
...
\subsection{PCBs}
A Printed Circuit Board (PCB) is a thin board made of alternating layers of conductive material (e.g., copper) and insulating substrate (e.g., FR-4). The conductive copper layers are laid out in tracks, also known as traces, and are used to connect electronic components together \cite{Grout2008DigitalCPLDs}.
...
A CNN \cite{Goodfellow-et-al-2016} is a type of neural network that is more effective at processing grid-like data, e.g., image data. In a CNN, a filter (or kernel) slides across the input data, performing an operation with the part of the data it's over at each step. The resulting number is stored as a feature.
...
Monte Carlo dropout is an ensemble approach that can be used to improve performance of neural networks. Random dropout is enabled during model inference, meaning the model output is non deterministic for the same input. By aggregating multiple predictions, the model accuracy can be improved \cite{Milanes-Hermosilla2021MonteClassification}.
...
\begin{itemize}
    ...
    \item \textbf{Absorption} - materials have an absorption coefficient, which measures how strongly they absorb light per unit distance.
...
The FlexIR wristband, shown in \cref{fig:FlexIR_wrist}, performs gesture recognition by measuring the transmission, reflection, and scattering of IR light through the wrist.
...
\begin{figure}[htp]
    \centering
    \includegraphics[width=0.48\linewidth]{stiffener.png}
...
Each LED group and photodiode must be individually controllable. To reduce the number of wires required between the wristband and microcontroller, we used 74HC595D shift registers for LED activation. Daisy chaining two 74HC595D chips allows for the control of 16 LEDs using 5 output pins of the microcontroller.
...
The transimpedance amplifiers convert the small currents produced by the photodiodes into larger voltages that can be measured by the microcontroller. The transimpedance amplifiers use TLV9062 op-amps configured with $100~k\Omega$ feedback resistors and 1~pF capacitors. The outputs of the transimpedance amplifiers are connected to the analogue input pins of the microcontroller.
...
\Cref{fig:rigid_pcb} shows an early prototype based on a rigid PCB, consisting of one shift register, one Darlington array, 8 visible light LEDs, and 8 IR photodiodes with transimpedance amplifiers. The visible light LEDs allowed easy testing of the LED control and an IR emitting torch was used to test the sensing circuit, before committing to the higher cost and lead time of a flexible PCB.
...
The data captured from GestIR and FlexIR is structured, grid-like data. The readings are also toroidal in structure - the wristband is circular (the first and last segments are physical neighbours), so the top and bottom, and the left and right, of the matrix can be considered adjacent. 

Convolutional neural networks (CNNs) perform well on structured, grid like data such as images. Because of this, they were expected to be most effective for this application. GestIR evaluated standard CNNs, toroidal CNNs that match the toroidal structure of the readings, as well as standard multi layer perceptrons (MLPs).
...
\Cref{fig:accuracy_all} shows the average accuracy of MLPs with Monte Carlo dropout trained on data from the $\pm 12^\circ$ and $\pm 60^\circ$ wristbands across all participants. To account for variation introduced by data partitioning, we trained five models on different stratified splits of the data. The bars show the mean model performance, with error bars representing the standard deviation. Overall, the $\pm 12^\circ$ wristband performed marginally better, averaging 81.6\% accuracy compared to 78.2\% accuracy with the $\pm 60^\circ$ wristband. However, the results were noisy, varying between participants, and a paired $t$-test revealed the difference in performance between the two wristbands was not statistically significant.
...
We trained models using data with only reflection features with a band width of 3. A paired $t$-test between these accuracies and the accuracies of models trained on all data was not significant for either wristband. This indicates that reflection features generated from photodiodes physically close to the active LEDs provide sufficient information and that transmission features may not be necessary for gesture recognition. Another paired $t$-test also revealed no statistically significant difference between the accuracies of the $\pm 12^\circ$ and $\pm 60^\circ$ wristbands, suggesting both LEDs were capable of producing informative reflection features.
...
The performance of the wristbands varied between participants, likely due to anatomical differences. One hypothesis was that wrist circumference was a factor in how effectively IR light penetrates tissue. However, no significant correlation was found between wrist circumference and model performance for any of the experiments. Further research investigating the effects of other anatomical differences is needed to determine the source of the variation between participants.
...
The first major achievement is the design, manufacture, and evaluation of a new flexible PCB based gesture recognition wristband. Previous designs of similar gesture recognition systems have been useful as research platforms, but lack in practicality and reliability. FlexIR's design has multiple benefits:
\begin{itemize}
    \item Wristbands can be produced using third party PCBA services. This makes high quality hardware more easily obtainable, reducing the skill level needed to run user trials and enabling future data collection and experiments on a wider range of participants without the overhead of building wristbands by hand.
    \item A wider range of SMD components can be compared to investigate the optimal properties for gesture recognition
    \item The reduced form factor and improved reliability move the device closer to a product capable of real world gesture recognition
...
\begin{itemize}
    ...
    \item Results suggest that narrow viewing angle LEDs with higher peak radiant intensities generate better features from deeper tissue, although further research is required to completely isolate this from LED package size
...
In the current design, the LEDs and photodiodes make direct contact with the wrist to allow for the comparison of the optical properties of components. Over long sessions, this becomes uncomfortable and can leave indentations in the skin. Encasing the wristband in a soft material, e.g., silicon, could improve the ergonomics. However, research is necessary into the effect of introducing additional materials between the sensing circuits and the skin.
...
The FlexIR wristband can be easily manufactured and assembled using third party PCBA services. This opens the door to larger user studies and machine learning experiments without the need for hardware expertise. There are many possible avenues for further investigation, such as:
\begin{itemize}
