...
Despite the toroidal CNNs more closely resembling the shape of the input data, the MLPs performed the best, achieving an average of 85.8\% test accuracy with 950nm LEDs\cite{diamand2025}. We briefly evaluated the standard and toroidal CNNs on FlexIR using the same parameters, and found similar results (\cref{fig:all_model_accuracies}). This supports GestIRs findings that high model complexity isn't necessary for accurate gesture recognition.
...
To generate data containing only reflection or transmission features, we set the relevant parts of the capture matrix to zero. \Cref{fig:heatmmaps} shows readings containing reflection and transmission features with band width 3 and transmission features with band width 5.
...
When IR light travels through biological tissue, three main phenomena occur: reflection off of boundaries between materials, absorption into materials, and scattering \cite{diamand2025}. Generally, the larger the distance between the source and detector on the surface of the skin, the deeper the detected photons travelled on their path through the tissue - Yang et al. \cite{Yang2007QuantitativeSpectroscopy} demonstrated that, on the forearm, a 2.5 mm source-detector distance captures light from only the skin and fat layers, whereas a larger source-detector distance of 30 mm captures light from deeper muscle layers. On the FlexIR wristband, the shortest distance between LEDs and photodiodes on the same segment is 10 mm. For transmission features with a band width of 3, the smallest LED-photodiode distance is 20 mm, and for transmission features with a band width of 5, the distance is 28 mm. Based on this, the models trained on transmission features rely on deep tissue interactions, whereas the models trained on reflection features use shallow reflections from skin and tendons.
...
Overall, the $\pm 12^\circ$ and $\pm 60^\circ$ angle LEDs achieved similar performance, suggesting that in general, viewing angle isn't a factor in how well IR sensing systems can classify gestures. However, when we forced the models to learn transmission features, the $\pm 12^\circ$ wristband performed significantly better than the $\pm 60^\circ$ wristband, showing that a higher radiant intensity is important when generating features from deep tissue. This is likely because the higher radiant intensity results in a higher photon density, which increases the number of photons that survive long scattering paths through deep tissue. Despite this, the overall accuracy of both wristbands was lower when trained on only transmission features, showing that reflection features which detected more superficial interactions with skin and tendons produced the most crucial data for gesture detection.
