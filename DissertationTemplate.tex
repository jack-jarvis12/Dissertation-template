...
\begin{table}[htp]
    \centering
    % Add vertical padding (default is 1.0)
    \renewcommand{\arraystretch}{1.5} 
    
    % Add horizontal padding (default is usually 6pt)
    \setlength{\tabcolsep}{10pt}
    \begin{tabular}{|p{0.4\textwidth}|l|l|}
        \hline
        \textbf{Experiment} & $\pm 12^\circ$ LED Accuracy & $\pm 60^\circ$ LED Accuracy \\
        \hline
        \textbf{All Features, 16 Gestures} & 89.0\% & 86.7\% \\
        \hline
        \textbf{All Features, 20 Gestures} & 81.6\% & 78.2\% \\
        \hline
        \textbf{Smartwatch Features \newline(Dorsal Side Only), 20 Gestures} & 67.3\% & 61.1\% \\
        \hline
        \textbf{Transmission Features \newline(Band Width 3), 20 Gestures} & 58.0\% & 31.4\% \\
        \hline
...
Models trained on data excluding the pinch gestures achieved average accuracies of 89.0\% and 86.7\% on the $\pm 12^\circ$ and $\pm 60^\circ$ respectively.
...
wristband). This indicates that reflection features generated from photodiodes physically close to the active LEDs provide sufficient information and that transmission features may not be necessary for gesture recognition.
...
We conducted a paired $t$-test to compare the average accuracies of each wristband. This showed a significant difference ($t$ = 5.362, $p <$ 0.001, $p_{adj}$ = 0.002, Cohen's $d$ = -1.617).
...
\autoref{fig:accuracy_smartwatch} shows the average accuracy of models trained using only data from photodiodes 8, 9, 10, and 11. The $\pm 12^\circ$ wristband had an average accuracy of 67.3\% and the $\pm 60^\circ$ wristband had an average accuracy of 61.1\%. This is a performance drop of 14.3\% and 17.1\% compared to models trained on all segments. A paired $t$-test between the accuracies for both wristbands revealed that the $\pm 12^\circ$ wristband performs significantly better on smartwatch features ($t$ = 2.773, $p$ = 0.020, $p_{adj}$ = 0.039, Cohen's $d$ = -0.836). 

\subsection{Confounding Variables}
\label{sec:confounding_variables}
\subsubsection{Order Effects}
The order participants completed the data collection sessions was counterbalanced - group A used the $\pm 60^\circ$ wristband first, and group B used the $\pm 12^\circ$ first. To check for order effects, we calculated the difference in performance between the wristbands for each participant and performed Welch's unpaired $t$-tests on the performance differences of groups A and B. \autoref{tab:order_effects} shows the results - the $t$-tests produced insignificant $p_{adj}$-values for all experiments, showing that there is no evidence that the order participants completed the sessions affected the results.
...
Permutation tests were used to investigate environmental effects because alternative tests, such as Kruskal Wallis H tests, rely on approximations that are unreliable at small sample sizes. The results of the permutation tests for $\pm 12^\circ$ and $\pm 60^\circ$ wristbands are shown in \autoref{tab:location_effects_12} and \autoref{tab:location_effects_60}. The $p_{adj}$-values are all insignificant, showing that there was no evidence any environmental effects on wristband performance. 
...
\begin{table}[htp]
    ...
    \begin{tabular}{lrrr}
        ...
        \bottomrule
    \end{tabular}
    \caption{Results of Kruskal Wallis H tests and permutation tests to investigate environmental effects for the $\pm 60^\circ$ wristband}
...
\begin{table}[htp]
    ...
    \begin{tabular}{|l|l|l|}
        ...
        \hline
        \textbf{All Features, Excluding Pinch Gestures} & 92.4\% & 91.8\% \\
        \hline
    \end{tabular}
